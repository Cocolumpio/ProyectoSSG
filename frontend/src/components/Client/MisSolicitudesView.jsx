...
const estadoConfig = {
  ...
  cancelado: { color: 'bg-red-500/15 text-red-300 border-red-300', icon: XCircle, label: 'Rechazado' }
};
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl sm:text-2xl font-bold text-white">Mis Solicitudes de Vuelo</h2>
          <p className="text-white/50 text-sm">Historial de tus solicitudes</p>
        </div>
        <button
      ...
      {solicitudes.length === 0 ? (
        <div className="bg-[#15151B] rounded-xl border border-white/10 p-12 text-center">
          <ClipboardList className="h-12 w-12 mx-auto mb-4 text-white/30" />
          <h3 className="text-lg font-medium text-white mb-2">No tienes solicitudes</h3>
          <p className="text-white/50 mb-4">Solicita tu primer vuelo de dron</p>
          <button
            onClick={onNuevaSolicitud}
        ...
        <div className="grid gap-4">
          {solicitudes.map((solicitud) => {
            ...
            return (
              <div
                key={solicitud.id}
                className="bg-[#15151B] rounded-xl border border-white/10 p-6 hover:shadow-md transition-shadow cursor-pointer"
                ...
                {selectedSolicitud?.id === solicitud.id && (
                  <div className="mt-4 pt-4 border-t border-white/5 space-y-3">
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div>
                        <p className="text-white/50">Inicio del proyecto</p>
                        <p className="font-medium">{solicitud.fecha_inicio_proyecto}</p>
                      </div>
                      <div>
                        <p className="text-white/50">Fin del proyecto</p>
                    ...
                    {solicitud.notas && (
                      <div>
                        <p className="text-sm text-white/50">Tus notas:</p>
                        <p className="text-sm text-white/80 bg-[#0F0F14] rounded-lg p-3 mt-1">
                          {solicitud.notas}
                        </p>
                      </div>
                    )}
                    ...
                    {solicitud.comentario_admin && (
                      <div className="bg-blue-500/10 border border-blue-500/30 rounded-lg p-4">
                        ...
                        <p className="text-blue-300">{solicitud.comentario_admin}</p>
