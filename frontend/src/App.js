      ...
      const proyectosDemo = [
        {
          ...
          coordenadas: { lat: 19.3586, lng: -99.2628 },
          fecha_inicio: "2024-01-15",
          fecha_fin_planeada: "2025-12-30",
          descripcion: "Construcción de torre de oficinas de 25 pisos"
        },
        {
          nombre: "Residencial Valle Dorado",
          ubicacion: "Interlomas, Estado de México",
          coordenadas: { lat: 19.4008, lng: -99.2612 },
          fecha_inicio: "2024-03-01",
          fecha_fin_planeada: "2025-08-15",
          descripcion: "Desarrollo residencial de 120 unidades"
        },
        {
          nombre: "Centro Logístico Norte",
          ubicacion: "Cuautitlán Izcalli",
          coordenadas: { lat: 19.6453, lng: -99.2195 },
          fecha_inicio: "2024-02-10",
          fecha_fin_planeada: "2025-06-20",
          descripcion: "Complejo de bodegas industriales"
        }
      ];

      for (const p of proyectosDemo) {
        const res = await axios.post(`${API}/proyectos`, p);
        const proyectoId = res.data.id;
        
        // Actualizar avance
        await axios.put(`${API}/proyectos/${proyectoId}/avance`, null, {
          params: { avance: Math.random() * 80 + 10 }
        });

        // Crear vuelos para cada proyecto
        const numVuelos = Math.floor(Math.random() * 3) + 2;
        for (let i = 0; i < numVuelos; i++) {
          await axios.post(`${API}/vuelos`, {
            proyecto_id: proyectoId,
            fecha_vuelo: `2024-${String(Math.floor(Math.random() * 8) + 1).padStart(2, '0')}-${String(Math.floor(Math.random() * 28) + 1).padStart(2, '0')}`,
            duracion_minutos: Math.floor(Math.random() * 40) + 20,
            area_cubierta: Math.floor(Math.random() * 15000) + 5000,
            num_imagenes: Math.floor(Math.random() * 800) + 200,
            volumetria: {
              excavacion: Math.floor(Math.random() * 5000) + 1000,
              relleno: Math.floor(Math.random() * 3000) + 500,
              materiales: Math.floor(Math.random() * 2000) + 300
            },
            notas: "Vuelo realizado con condiciones óptimas"
          });
        }
      }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h1 className="text-2xl font-bold text-white">DroneBuild Pro</h1>
                <p className="text-sm text-slate-400">Gestión de Construcción con Drones</p>
              </div>
            </div>
